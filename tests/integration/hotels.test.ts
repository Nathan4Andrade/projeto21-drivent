...
describe('GET /hotels/:hotelId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 400 when the hotelId is not a number', async () => {
      ...
      const response = await server.get('/hotels/true').set('Authorization', `Bearer ${token}`);

      expect(response.status).toBe(httpStatus.BAD_REQUEST);
    });

    it('should respond with status 404 when there is no hotel with specified id', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeA(false, true);
      ...
      const response = await server.get(`/hotels/2325`).set('Authorization', `Bearer ${token}`);
